...
settingsSchema.statics.setSetting = function (category, key, value, description = '') {
    return this.findOneAndUpdate({ category, key }, { value, description }, { upsert: true, new: true });
};
...
settingsSchema.statics.bulkUpdate = function (settingsData) {
    const operations = Object.entries(settingsData).flatMap(([category, categorySettings]) => Object.entries(categorySettings).map(([key, value]) => ({
        updateOne: {
            filter: { category, key },
            update: { $set: { value } },
            upsert: true
